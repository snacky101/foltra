...
3. 확장 카드의 **설정**에서 연결 확인, 덱, 데이터베이스와 앞면·뒷면 컬럼을 선택하고 설정을 저장합니다. DB 없이 태그 블록만 사용할 수도 있습니다.
