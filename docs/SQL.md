...
- 컬럼 이름을 바꾸면 저장된 노트의 `foltra-sql`과 SQL 형식 `foltra-query`에서 해당 컬럼 참조도 함께 갱신합니다. 변경할 쿼리 수를 속성 편집에서 확인할 수 있습니다. 문자열 값·주석·일반 코드 예시는 바꾸지 않습니다. DB 자체의 이름 변경은 아직 이 대상이 아니므로 기존 SQL의 테이블 이름을 직접 수정해야 합니다.
- 값이 없는 컬럼은 `NULL`입니다. 비교는 `= NULL` 대신 `IS NULL` 또는 `IS NOT NULL`을 사용합니다.

컬럼 이름 변경은 참조할 테이블과 컬럼을 확인할 수 있을 때 적용합니다. 모호한 참조, `USING`/`NATURAL JOIN`, 바깥에서 사용하는 하위 쿼리의 별칭 없는 계산식 등은 해당 노트와 수정 방법을 안내하고 전체 변경을 중단합니다. 테이블 별칭·명시적인 `ON` 조건·계산식의 `AS` 별칭으로 참조를 명확히 한 뒤 다시 적용할 수 있습니다. 검사 이후 노트나 DB가 바뀌면 다시 확인해야 하며, 기존 쿼리나 최신 편집 내용을 자동으로 덮어쓰지 않습니다. 노트 밖의 스크립트·확장 코드에 직접 작성한 SQL은 자동 변경 대상이 아닙니다.
...
필요하면 각 테이블의 메타데이터 컬럼도 조회할 수 있습니다. `__id`는 자동 생성되는 고유 행 ID(PK), `__note`는 연결된 본문 노트 ID 또는 `NULL`, `__created_at`·`__updated_at`은 생성·수정 시각 문자열입니다. 제목 컬럼은 중복·빈값을 허용하므로 PK가 아니며, 제목 셀 옆에서 해당 행의 본문 노트를 엽니다. 컬럼 이름을 바꿔도 행 ID와 본문 연결은 유지됩니다. 모두 SQL 타입은 `VARCHAR`이며, 날짜 계산이 필요하면 `"__created_at"::TIMESTAMP`처럼 변환합니다. `SELECT *`에는 이 메타데이터도 포함되므로 보여 줄 컬럼을 직접 고르는 편이 읽기 좋습니다.
