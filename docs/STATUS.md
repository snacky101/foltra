...
## 2026-09-18 preview.2 배포 준비

앱 업데이트·Git 동기화 확장·휴지통 관리·macOS 텍스트 대치와 Makefile을 `0.1.0-preview.2`로 함께 배포합니다. npm/Cargo/Tauri 버전 일치와 배포 범위를 확인했고, 전체 1,402개 테스트·정적 검사·macOS debug 앱 빌드는 통과했습니다. GitHub Actions에서 최적화된 앱·DMG·CLI·서명된 업데이트 파일을 만들고 게시된 파일과 첫 피드를 검증한 뒤, 기존 `preview.1` 릴리스를 제거할 예정입니다. 현재 단계의 실제 게시·다운로드 검증 결과는 완료 후 갱신합니다. 아래 작업별 미커밋·미배포 문장은 해당 작업 완료 당시의 이력입니다.
