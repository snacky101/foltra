...
## 2026-09-21 preview.6 릴리스 대상

`0.1.0-preview.6`에는 앱에 동봉되는 CLI·설정의 터미널 명령 설치와 배포 작업 간소화를 포함합니다. npm/Cargo/Tauri 버전을 일치시켰으며 앱 기능 소스는 직전 전체 1,716개 검사·정적 검사·개발 앱 빌드 상태 그대로입니다. 정확한 태그 커밋의 원격 검사와 DMG·업데이트 파일·CLI 검증을 거쳐 게시합니다. 배포 검증 자료는 `test-results/release-preview6/`에 보관하며 아래 작업별 미배포 문장은 해당 작업 시점의 이력입니다.
