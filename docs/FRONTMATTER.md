...
macOS에서 **Cmd+;**를 누르면 새 속성 행이 열리고 이름 입력칸으로 이동합니다. 속성이 없는 노트에도 본문을 보존한 채 속성 영역을 추가합니다. Live Preview·원문·읽기 모드에서 사용할 수 있으며, 설정에서 속성 추가 명령의 단축키를 바꾸거나 해제할 수 있습니다.

YAML을 직접 편집하려면 노트 제목 아래 오른쪽의 속성 아이콘 또는 명령 팔레트의 **노트 속성 · frontmatter 편집**을 사용합니다. 속성이 없는 노트에는 `tags: []`가 들어 있는 영역을 맨 위에 추가합니다. 설정에서 이 명령에도 일반 단축키와 leader 조합을 지정할 수 있습니다.
