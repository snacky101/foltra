...
Installation does not enable code. Activation requires explicit approval of the package's requested permissions, bound to its exact SHA-256 digest and canonical vault path. Trust is stored in the device's application data, outside the vault; importing or synchronizing a vault does not activate its code. Changed code or permissions invalidate approval. Disabling/removing a package stops event delivery and removes its commands/views from the active UI. Plugin settings and created notes/records survive removal.
...
Existing installed packages are not silently replaced by catalog updates. The host provides a narrow compatibility mapping for Anki 1.0.0: its existing declared `sync` view already contains its settings and is accessible from the settings button without reinstalling or changing its approval. New packages should declare `settingsView` explicitly. Other views are not automatically treated as settings.

## Acceptance checks

Anki is the sole bundled plugin. Keep SDK regression packages under `tests/fixtures/plugins/`, outside the app catalog. Verify install/approval/enable/disable/remove, plugin settings, custom views and actions, note/DB change events, CLI command execution, Leader/regular shortcut discovery, revision conflicts, stale editor results, changed-package trust rejection, permission denial, bounded loops/memory/output, and legacy package compatibility. Do not equate these checks with full Obsidian API compatibility, independent security auditing or verified behavior on untested platforms.
