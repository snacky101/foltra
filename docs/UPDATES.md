...
`node --test scripts/release-update.test.mjs`는 버전 정렬, 태그·버전 일치, 피드 역행 거절, manifest URL, 파일 누락·변조, 압축 경로와 Minisign 검증을 검사합니다. 실제 Tauri CLI로 임시 키를 생성해 서명한 파일의 양성·변조 음성 테스트도 포함합니다. 테스트 키는 종료할 때 지웁니다.
